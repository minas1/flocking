...
public class StarlingHunt : SingleBirdState
{
	List<Collider> colliders;
	string[] enemyTags;
	
	public StarlingHunt(Bird bird, string[] _enemyTags) : base(bird)
	{
    ...
    public override void Update(float dt, Bird bird)
	{
		...
		if( hasVisionOf(bird, enemyTags[0], new string[]{"Ground", "Untagged"}, colliders) )
		{
			...
			if( distance <= StarlingAttack.ATTACK_DISTANCE )
				bird.state = new StarlingAttack(bird, birdToAttack, enemyTags);
		}
		else
		{
