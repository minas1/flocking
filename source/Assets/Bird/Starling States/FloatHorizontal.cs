...
public class FloatHorizontal : MultipleBirdState
{
    ...
    {
        return Vector3.Distance(charPos, pos) <= maxDistance && Mathf.Abs(charPos.x - pos.x) <= 5f;
    }
    ...
    public override void Update(float dt)
    {
        ...
        for(int i = 0; i < entries.Count; ++i)
        {
            ...
            if( Mathf.Abs(pos.x - flockCenter.x) > 75f || Mathf.Abs(pos.y - flockCenter.y) > 10f )
            {
                var path = new Path();
                ...
                separation.aknnApproxVal = 2f;
                cohesion.aknnApproxVal = 10f;
